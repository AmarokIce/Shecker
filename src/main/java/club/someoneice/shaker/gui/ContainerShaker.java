...
import club.someoneice.shaker.ShakerItem;
...
public class ContainerShaker extends AbstractContainerMenu {
    ...
    public ContainerShaker(int id, Inventory inventory) {
        super(ShakerMain.SHAKER_GUI.get(), id);
        ...
        item.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(it -> {
            for (int i = 0; i < 2; i ++) for (int o = 0; o < 4; o ++)
                addSlot(new SlotInput(it, o + i * 4, 31 + o * 18, 16 + i * 18));
            addSlot(new SlotResult(it, 8, 132, 24));
        });
    }
